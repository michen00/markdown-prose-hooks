...
The two hard-break syntaxes are not interchangeable outside this tool. Every renderer treats two trailing spaces as a break, but ordinary tooling deletes them and treats the deletion as a cleanup rather than an error, so a break can lose its marker and be joined by the next run. A backslash survives that tooling and can be read in the source, but it is a CommonMark addition rather than original Markdown syntax, and Python-Markdown [does not implement CommonMark](https://python-markdown.github.io/#goals), so it renders the backslash literally and no break at all. Keeping a two-space break under `pre-commit` needs a flag, covered in [As a pre-commit hook](#as-a-pre-commit-hook).
