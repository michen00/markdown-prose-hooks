...
The match is exact and it is on the whole of what the comment holds, so a comment carrying more than the one word is prose about the tool rather than an instruction to it. That is the first of three clauses, and where the comment ends is the second rather than no part of the rule: a comment that opens on one line and closes on a later one is the directive when everything between the delimiters is the word and nothing else, and when the closing delimiter ends its line. Text after that delimiter makes it a sentence that happens to close a comment, which is what the one-line form rejects for the same reason and what prettier joins too, measured. The third clause is about quote depth and is a paragraph below. Inside them the form below says exactly what the one-line form says, blank lines inside it included, and it was the one shape where this tool silently lost a break an author had marked.
...
One inside a fenced code block is inert, which is what lets this section print it, and so is one inside front matter or inside another comment — a comment nested in a comment is content the outer one carries, which is not the bare word. A quoted fence and a quoted HTML block make it inert the same way, at whatever depth they are written at. Blockquote markers come off every line of the comment, the whole stack of them rather than one level at a time, so `> <!-- unwrap-ignore -->` and its multi-line form both exempt the quoted paragraph from inside the quote rather than from outside the block it governs, and both keep governing the paragraph below them however deeply the comment itself is quoted — two and three levels down carry cases, and four answers the same way, measured, in both forms and over an unquoted paragraph. How deeply the *paragraph* is quoted is a separate question with a blunter answer: prose two or more quote levels deep is not joined here at all, with a directive above it or without one, so a directive written at that depth changes no answer and the case pinning this says so rather than appearing to pin the directive. Every line the comment covers has to carry the same number of those markers: one opening at `>` and carrying on at `>>` is not a directive, because a comment whose depth wanders part-way is not one comment's worth of content, and taking off whatever each line happened to carry would exempt a paragraph nobody marked. It is the count and not the characters, so a comment written `>>` on one line, `> >` on the next and `>  >` on the last is one comment at one depth. The directive is unscoped, where prettier's marker is a sibling in the tree it sits in: this one exempts the next paragraph in whatever container that paragraph turns out to be in, so `> <!-- unwrap-ignore -->` above an unquoted paragraph exempts it and prettier joins the same paragraph, measured. That is the other place the two tools part, and it parts the other way from the one above.
...
The names follow `prettier-ignore-start` and its partner, so a reader who knows that pair knows this one. Both markers are matched the same way the single directive is — exactly, on the whole of what the comment holds, with the closing delimiter ending its line and the whole blockquote stack off every line of it — so both may be written across a multi-line comment too, and a region opened one way is closed the other. Both are inert inside a fenced code block — a quoted one included, however deep the quote — inside front matter, and inside a comment carrying anything besides the marker, which is what lets this section print them. A region suspends the transform rather than narrowing it, so a wrapped list item or blockquote inside one is left as written too, and neither count moves, so `--fail-on-change` passes a file whose only prose is exempt.
