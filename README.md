...
repos:
  - repo: https://github.com/michen00/markdown-prose-hooks-py
    rev: v0.2.1 # Use the latest version
    hooks:
...
```yaml
- uses: michen00/markdown-prose-hooks@v0.2.1
  with:
    write: 'false'
    fail-on-change: 'true'
```
...
jobs:
  propose:
    uses: michen00/markdown-prose-hooks/.github/workflows/unwrap-propose.yml@v0.2.1
```
...
jobs:
  comment:
    ...
    permissions:
      actions: read
      pull-requests: write
    uses: michen00/markdown-prose-hooks/.github/workflows/unwrap-comment.yml@v0.2.1
```
