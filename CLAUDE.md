...
- The Rust jobs gate a pull request: `rust-lint`, every `rust-test` entry and every `parity` entry are required contexts. `rust-test-stable` deliberately is not, for the reason its own comment gives: its toolchain ref floats, so an upstream release could turn it red with nothing here at fault and block every open pull request. A matrix axis on `rust-test` would have made it required by that first rule and renamed three required contexts on the way, which is why it is a separate job. `coverage` deliberately is not either, and the comment above that job says why — it mints its credential through OIDC, which a pull request from a fork cannot be granted, so requiring it would block an outside contribution permanently. `mirror-identity` is not required either, for the reason its own comment gives.
