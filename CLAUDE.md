...
**Nothing but `smoke.yml` tests what a registry serves.** Every other job builds the thing it tests, so a wheel missing a module, or a crate that will not compile from its own package, would publish green. That workflow installs from PyPI and from crates.io, checks the released binaries against `SHA256SUMS`, and runs the CLI tier against all three; the release flow calls it after every publish, and a weekly schedule and a manual dispatch each call it again for a tag already released. It gates none of the publishes, because by the time it runs the version number is spent; the release flow's `alias` job does wait on it, so a red run leaves `v0`, the alias it otherwise moves to the newest release, on the previous one. It takes the harness from the ref it runs on and the corpus from the tag, so a run of an already-released tag judges that version against the specification it was published against, using the current harness rather than the one the tag carries.
...
- The Rust jobs gate a pull request: `rust-lint`, every `rust-test` entry and every `parity` entry are required contexts. `rust-test-stable`, `coverage` and `mirror-identity` are deliberately not required, each for the reason its own comment gives, and requiring one would block pull requests for causes outside this tree. `rust-test-stable` is a separate job rather than a matrix axis on `rust-test`, because an axis would have made it required by that first rule and renamed three required contexts on the way.

## More than one agent works this repository at once
...
- The benchmark notebook times processes a few milliseconds long, so do not re-execute it while another session is working, and do not run a suite alongside a notebook run: the notebook reports a busy machine as a failed check.
...
`typos` and `codespell` both run: `typos` catches misspellings and splits identifiers, `codespell` adds the American-spelling dictionary `typos` has no equivalent for. House spelling is US English.
...
- **The commit-message gate reads the message, not only the diff.** `codespell` and `gitlint` both run at `commit-msg`, so a message quoting a corpus fixture name can be rejected as a misspelling — a name like `note.md` with a letter dropped, or a glob putting `?` inside a word. Describe such a token instead of quoting it.
