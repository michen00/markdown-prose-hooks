...
The crate's `include` key is load-bearing for the `-rs` mirror rather than a tidiness measure. Without it the crate ships the whole repository — 431 files and 1.4 MiB — so a consumer who cloned a 10K mirror would pull all of it down at `cargo install` time, and the mirror would have bought them nothing.
