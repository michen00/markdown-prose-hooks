...
The order is therefore forced rather than chosen. `v0.0.1` publishes both packages and so proves the release machinery, and is also the first moment a crate exists for the `-rs` mirror to depend on; the mirrors are then built and a `v0.0.2` tag exercises them end to end; `v0.1.0` is the first tag anybody is pointed at, with the ids served only from the mirrors and this repository's manifest gone.
...
The crate's `include` key is load-bearing for the `-rs` mirror rather than a tidiness measure. Without it the crate ships the whole repository — 411 files and 1.2 MiB — so a consumer who cloned a 10K mirror would pull all of it down at `cargo install` time, and the mirror would have bought them nothing.
