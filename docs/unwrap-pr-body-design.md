...
A caller triggered by `pull_request_target` reads its workflow file from the default branch, so a pull request here cannot exercise it. CLAUDE.md records the same limitation for the comment half of the existing pair. A workflow that cannot be run can still have its wiring asserted, and `tests/test_workflow_contracts.py` asserts it: that the trigger is the intended one, that the permissions are the narrow set listed above, that no step checks out or executes anything from the pull request, that the body never reaches a shell through expression interpolation, and that the two halves name one report marker. It covers the existing pair as well.

Using this repository as the first consumer rested on a measurement that no longer holds. On 2026-09-14 three pull requests were open here, and the transform changes two of them, removing 38 manual line breaks between them; both implementations return the same bytes for each, and `gh pr view <n> --json body --template '{{.body}}' | unwrap-markdown-prose -` reports it for any of them. The reporting half therefore runs here, called by path from `.github/workflows/prose-body.yml`, and what it reports is a verdict on wrapped prose rather than a demonstration that the workflow starts.

The editing half is not enabled alongside it. It writes once per pull request, and `pull_request_target` means the pull request adding it cannot exercise it, so its first run would be a live body rewritten by a job nothing here had yet executed. It waits on what the report finds, which is the escalation [the three modes](#the-three-modes) describes. Genuine verification of that half remains the contract tests together with one deliberately wrapped body on a pull request in `unwrap-fork-pair-check`.
...
An edit is scoped by who wrote the prose rather than by which surface it sits on. The only way to keep a line break is the `unwrap-ignore` comment, and someone leaving a comment has no way to learn that before a break of theirs is joined: the comment box says nothing about it, and the rewrite arrives with no preview and no undo they would know how to reach. A body's author opened the pull request the workflow runs on. A commenter consented to nothing, so a comment somebody else wrote is not the transform's to rewrite. The report is not obviously theirs to receive either, since it is advice about their writing on a thread about something else.

The new surface reports before it edits, which is the escalation [the three modes](#the-three-modes) already describe.

The trigger is `issue_comment`, and the caller supplies it. A conversation comment changes under that event and under no other, and the event carries the comment in its payload, so the workflow keeps the property the editing workflow rests on: it takes no text as an input and reads what it transforms from the event. Sweeping every comment on each pull request event would examine comments nobody had touched, and would end the rule that a run with nothing to report calls no API at all.

Three things come with that trigger. It fires for issues as well, so the job tests `github.event.issue.pull_request` before it acts. GitHub reads the workflow from the default branch, as it does for `pull_request_target`, so a pull request cannot exercise it. And a comment lives in the base repository rather than in a fork, so the token is whatever the caller granted, whoever wrote the comment; GitHub's documentation does not say that in as many words, so the trigger is treated as elevated and held to the same three invariants the body surface has. One name has to cover both triggers in the concurrency group, because the number is `github.event.pull_request.number` under one and `github.event.issue.number` under the other, and two halves in separate groups would race.

Posting the report fires `issue_comment` in turn, and two independent things keep that from looping. GitHub starts no run for an event its own `GITHUB_TOKEN` caused, which is what the editing workflow already depends on. And the report is written by a bot rather than by the pull request's author, so the rule above puts it out of scope even where a run does start. A consumer driving this from a personal access token loses the first and keeps the second.

The bot test needs no strengthening, and the reason is recorded here so that `user.type` is not later mistaken for one. It does two jobs: it skips a pull request a bot opened, and it finds the reporting workflow's own comment so that comment can be updated or withdrawn. A `User`-typed automated account such as `codecov-commenter` breaks neither, because it opens no pull requests and posts no marker. The rule it would have broken — skip a comment a bot wrote — is unnecessary once an edit is scoped to the pull request author's own text, since no bot's comment is that. Every one of the 5 comments the measurement found was written by the author of the pull request it sits on, so authorship reaches each of them without consulting a type. What is left is a limit rather than a defect: a pull request opened by a `User`-typed automated account would not be skipped, and the only accounts that have opened one here are `michen00`, `dependabot[bot]` and `pre-commit-ci[bot]`, the last two both reporting as bots.
