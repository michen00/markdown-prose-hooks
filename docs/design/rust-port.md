...
What the channel does need is a fallback rather than a choice. A runner on a platform with no published binary must still work, so the action detects and falls back to `pip install` of the Python package. An `implementation` input taking `auto`, `rust` or `python` sits on top of that, defaulting to `auto`: the explicit values exist for pinning behavior deliberately or for isolating a suspected divergence, not for routine use. The `python-version` input then governs only the Python path, and says so. All of it ships. `action.yml` resolves the input, downloads the binary for the runner's target, and declines to run one it cannot verify -- a manifest that is absent, or one that does not name the asset, is an uncovered platform and takes the fallback, while past that point a download that fails or a digest that disagrees is fatal, because the manifest named the asset and so a broken or tampered release is the remaining explanation. Step 14 below records the switch, which retired the `DEVIATION` comment that had named this paragraph as the shape it was waiting for.
...
The measurement that decides it is what `pre-commit` downloads, and it is not what an earlier draft of this section assumed. `pre-commit` shallow-clones a hook repository at `--depth=1` for a tag rev, so history is not the cost and the tree at that rev is. That tree was 1224K when this was written, 1442K across 432 files when it was next measured, 1481K across 434 files on 2026-08-21, and 1560K across 612 files on 2026-09-03, of which 532 are corpus fixtures. The notebook and the corpus are what grew it, and every re-measurement so far has strengthened the same conclusion rather than weakening it. A Python shop pinning this repository takes 188K of Rust it will never read; a Rust shop takes the Python package and the same fixtures. The mirrors deliver 75K and 12K, built from this tree on the same date.
...
Both directions were verified rather than assumed, because the opposite was asserted first and turned out to be false. Cargo's target discovery is extension-scoped: it takes `lib.rs`, `main.rs`, and whatever they `mod`, and a sibling directory named `markdown_prose_hooks` is invisible to it. Hatchling's `packages = ["src/markdown_prose_hooks"]` names its directory explicitly, so a built wheel contains the three Python files and no `.rs` at all. In `tests/`, cargo sees one target and pytest sees one test.

The crowding argument does not survive contact with the actual file count. The Python package is `unwrap.py`, `__init__.py` and `__main__.py`, and it is one directory in `src/`; `ls src/` prints what sits beside it. A dozen Rust files beside one package directory is a normal `src/`, and separating them would buy tidiness at the cost of five manifest lines that can drift.

## Modules

The Python is one long module: `unwrap.py` was 1204 lines when this was written and is 1517 on 2026-09-03. That suits Python and does not suit learning Rust, so the port decomposes into units that can each be understood and tested alone.
...
**A line boundary is `\n`, `\r\n`, or `\r`.** `str.splitlines` recognizes eleven, and the extra eight produce a bug rather than a feature: `alpha wrapped\vbeta wrapped` becomes `alpha wrapped beta wrapped`, joined as two prose lines *and* with the `\v` deleted, because the join calls `strip()` and `\v` is whitespace. Silent character deletion is worse than not unwrapping. The Python narrows to the three Markdown boundaries, which was measured against the whole corpus before being chosen: over the 51 cases the tree held then — inputs and expected outputs both — plus every tracked Markdown file in the repository, 106 distinct files at that point, the narrowed splitter produced byte-identical output and identical counts. Two cases have since pinned the narrowing from the other side, naming a vertical tab and a line separator as characters that are not boundaries, so the corpus now states the rule rather than merely not contradicting it. The narrow rule is also the version-stable one, since the boundary set is a property of the interpreter rather than of this tool.

**An ordered list marker is ASCII `0-9`.** `\d` on a `str` pattern is the runtime's `Nd` category, and the fourth change is the one the port could not have merely ported: the Python did not agree with itself. `\d` selects 650 code points on 3.10's Unicode 13.0, 680 on 3.13's 15.1 and 760 on 3.14's 16.0, and all three interpreters are in this repository's own test matrix, so "match the Python" was not a defined target — a second implementation would have had to pin a Unicode version to agree with any one of them. CommonMark settles it in the other direction anyway: an ordered list marker is "a sequence of 1–9 arabic digits (0-9)", so no renderer ever read `١. x` as a list, and matching one only declined to unwrap prose that renders as prose. Measured over every distinct line in both corpus tiers, narrowing `_MATCH_LIST`, `_MATCH_LIST_MARKER` and `_MATCH_TIMESTAMPED_SPEAKER_HEADING` to `[0-9]` changes nothing the corpus already pins. It also deletes a 61-range table from the Rust in favor of one comparison, and the timestamp pattern was internally inconsistent before it: its speaker half is `[A-Z]`, ASCII by construction.
...
## The fifth specification change, which an accepted divergence had been standing in for

The four changes above all end the same way: the specification moves, both implementations follow, and the corpus pins the result. This section recorded one question that was held not to be able to end that way, and the holding was wrong. It is kept because the shape of the error generalizes further than the case does.

argparse decides whether a `-`-leading token is an option or a positional partly by asking whether it looks like a negative number, and it asks its own `_negative_number_matcher`. Through 3.13 that pattern is `^-\d+$|^-\d*\.\d+$`, whose `\d` is the same Unicode `Nd` category the fourth change removed from this tool's own patterns, carrying the same defect: a different set of code points on every interpreter in the matrix. So `--json -١٢` bound `-١٢` as a *path* under CPython and rejected it as an unknown option under the Rust, whose class is ASCII.

That was accepted rather than closed. The pattern belongs to the standard library, so narrowing it the way `_MATCH_LIST` was narrowed looked unavailable; overwriting it was considered, because it is an instance attribute, and was rejected as trading a rare documented divergence for a silent breakage on any interpreter that renamed or restructured a private attribute. The boundary was then stated in prose — every ASCII spelling agrees, only a non-ASCII digit differs — with one corpus case pinning the accepting half and nothing pinning the boundary itself.

**The rejected risk was a rename, and what happened was a value.** 3.14 kept the attribute and replaced its pattern with `-\.?\d`, which stops at the first digit and never reads the rest of the token. `-1a` and `-5.` are ASCII spellings, named in this section as agreeing, and they stopped agreeing: each is a path under 3.14 and an unknown option under 3.13 and under the Rust. `--write -1a note.md` therefore reported an error on one supported interpreter and silently formatted the named file on another. Nothing went red, because the section that promised those tokens agreed had pinned no case naming them.

So the divergence was not narrow, and it was not stable, and its width was a property of whichever interpreter was installed. The resolution is the one the other four changes take: the rule is the specification's rather than the runtime's. It is a dash followed by ASCII digits, or by ASCII digits around a point, and nothing else in the token; the Rust already held it, and the Python sets `_negative_number_matcher` to `^-[0-9]+\Z|^-[0-9]*\.[0-9]+\Z` at parser construction. That end anchor is `\Z` rather than `$` because `$` in Python matches before a token's final newline as well as at the end of it, which left a digit run ending in a newline a path there and an unknown option here. Every spelling now agrees, ASCII and not, and four cases pin both directions — `a-negative-number-argument-is-a-path` for the accepting half, and `a-number-like-token-with-a-letter-is-an-option`, `a-number-like-token-with-a-bare-point-is-an-option` and `a-non-ascii-digit-token-is-an-option` for the rejecting one. The last of those is the case that had been called unwritable. The newline tokens are not among them, because `argv` in that tier splits on whitespace and so cannot name one; they are pinned in both unit suites instead, over the same vectors — `test_the_negative_number_rule_is_the_tools_own` and `the_negative_number_rule_is_ascii_and_says_so`.

The private attribute is still a private attribute, and the original objection to touching it is answered rather than dismissed: an interpreter that renames it takes its own rule back, and the three rejecting cases are what say so — on that interpreter, in both implementations' terms, at the tier whose job is to notice. That is the generalizable part. A rule deferred to the runtime is not a rule two implementations share, whatever is written about how narrow the deferral is, and a boundary stated only in prose moves without anything noticing. The corpus is what makes the difference between a divergence that is documented and one that is merely described.

The practical reach was always small — a file literally named `-١٢` or `-1a` passed as a bare argument, unreachable through the `pre-commit` and Action channels, which never pass a `-`-leading token, and reachable past either implementation with `--` in front of it. Small reach is why it survived four releases, and is not a reason it should have.
...
14. Release plumbing: a per-architecture build matrix publishing prebuilt binaries for six targets, `action.yml` switched to download one instead of provisioning Python, and the four hook ids. The matrix and the ids shipped at `v0.0.1`, and the `action.yml` switch followed, which retired the `DEVIATION` comment that had named it.

Step one is first because the gap was real and open when this was written: the `exclude` named `corpus/cases/` specifically, so the CLI tier's fixture Markdown would have been rewritten by this repository's own hook the moment it landed. Widening the pattern before the directory exists costs one character and closes a window rather than discovering it.
...
**The region form owned the two hard questions, and both were decided against measurement.** `<!-- unwrap-ignore-start -->` and `<!-- unwrap-ignore-end -->` are named after prettier's pair, which is what made shipping the line form first safe: the region form needed no rename when it arrived. Neither question reached the line form, which is why the halves had been taken in this order.
...
Inertness cost one fix the corpus could not have found, and then cost it again one quote level down. Both markers are inert inside a fence, inside front matter, and inside a comment carrying anything besides the marker, and the region form gets that from guard order the way the line form does -- but a fence opened *inside* a region has to be tracked for the ordering to hold, and the first version tracked only an unquoted one. A closing marker inside a blockquoted fence therefore ended the region early, joining text an author had marked as unjoinable, which is the damaging direction. Both implementations agreed on it, so parity could never have caught it and neither could the fuzzer; it was found by probing the built binaries against inputs no case named. A case names it now.

The second time was the same shape with one more `>`. A quoted fence was armed from a line with a single blockquote marker peeled off, while every marker is read with the whole stack off, so two levels down no fence was tracked at all and everything the guard order was supposed to make inert was live inside quoted code -- a closing marker ending its region early again, and now a bare directive exempting a paragraph from inside a printed example. Arming reads the whole stack now, the way the markers do, and what closes a quoted fence or a quoted HTML block is read at the depth that armed it: a closing fence one level deeper is the code it looks like. A line quoted *less* deeply than the container ends it outright instead, the way a line carrying no marker at all already did — that is the same event written with one marker rather than with none, and gating the close on the container's depth without gating the drop the same way left the two meaning different things, with a live marker under the shallower one read as code and an author's break joined away. CommonMark ends the inner quote at such a line too, so a fence line one level up opens a fence of its own there in both readings, and the drop reproduces that by reprocessing the line. A quoted HTML literal drops the same way, a comment included: CommonMark ends every container at such a line, and exempting the comment let a directive under it close the old comment instead of being read. A literal still closes on its delimiter at its own depth or deeper, as CommonMark closes it, and the delimiter is read under the depth that armed it rather than under the whole stack, because `>` closes a declaration and is also a marker. A raw HTML block is the one place that rule knowingly parts from CommonMark, which ends such a block at its closing tag however deeply that line is quoted and would have the prose below it joined; a container model coarser than CommonMark's should spend the difference on declining rather than on joining, and a case records the choice rather than leaving it to read as an oversight. The drop at a shallower line adds a second coarseness, and it falls the way that principle asks: a line such as `> </div>` opens a block of its own in CommonMark and arms nothing here, so a marker under it is live here and dead there, and the paragraph below is left alone rather than joined -- measured over every depth pair for a fence, a raw and a non-raw HTML block and each literal kind, with a plain line or a closer quoted less deeply, as deeply or more deeply, where the joining direction is the raw block and nothing else. Both depths and both directions carry cases, and the fragment bank gained a fence and an HTML block two levels down, each also with a line quoted less deeply under it, and the same shape opened by a processing instruction, which is the one hazard class neither parity nor the fuzzer can reach on its own: the implementations agree on the wrong answer, so only a document somebody wrote down finds it.

Container scoping was declined for a reason the line form had already settled: it is unscoped, exempting the next paragraph in whatever container that paragraph turns out to be in, so a region that counted markers only against their own container would have made the two halves disagree about containers for no gain. `ruff` is the one tool measured that scopes structurally, and at a document's top level that answer collapses into the end-of-file one anyway.
