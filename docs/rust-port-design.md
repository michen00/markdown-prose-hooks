...
The crate's `include` key is load-bearing for the `-rs` mirror rather than a tidiness measure. Without it the crate ships the whole repository — hundreds of corpus fixtures, the Python package, the docs and the workflows, well over a megabyte — so a consumer who cloned a 10K mirror would pull all of it down at `cargo install` time, and the mirror would have bought them nothing.
...
**The region form owned the two hard questions, and both were decided against measurement rather than recollection.** `<!-- unwrap-ignore-start -->` and `<!-- unwrap-ignore-end -->` are named after prettier's pair, which is what made shipping the line form first safe: the region form needed no rename when it arrived. Neither question reached the line form at all, which is the argument for having taken the halves in this order rather than a claim that what was left was easy.
