...
- [ ] The notebook's deviation list promises more than it prints. Its lead-in says each file records "what it is waiting on", but the render takes the second and third sentences of each comment block and the blocker is the first, so the one thing the lead-in promises is the one thing omitted. Widening the slice changes what the notebook prints, which is why this is written down rather than done in passing.
