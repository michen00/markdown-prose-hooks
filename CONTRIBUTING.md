...
Marketplace needed a browser once and does not need one again. Creating [the listing](https://github.com/marketplace/actions/unwrap-markdown-prose) is a checkbox on the release form, and it asks for a two-factor confirmation that `release.yml`, which creates its release through the API, cannot give. After that first publish the listing follows the newest release on its own: the evidence is [boilerplate-sync](https://github.com/michen00/boilerplate-sync), whose listing advertises a version whose GitHub Release a bot created through the releases endpoint with nobody at a browser. So a release here is expected to move the listing without a step, and what to do after one is confirm that it did rather than go and tick something. If it ever stops following, the damage is bounded rather than silent — a stale snippet pins a version tag that is now immutable, so it resolves and runs, and the readme points at `@v0` regardless.
