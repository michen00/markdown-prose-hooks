...
Marketplace is the one step the tag does not carry. Publishing the release to [the listing](https://github.com/marketplace/actions/unwrap-markdown-prose) is a checkbox on the release form, it has to be selected again on every release, and it cannot be automated: Marketplace asks for a two-factor confirmation that only a browser can give, so `release.yml`, which creates its release through the API, will never move the listing. Whichever version was last ticked is the one Marketplace advertises. Forgetting is bounded rather than silent — a stale snippet pins a version tag that is now immutable, so it resolves and runs, and the readme points at `@v0` regardless.
